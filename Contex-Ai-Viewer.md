...
Если искать для этого знакомую инженерную аналогию, то ближе всего здесь мышление в терминах **state-driven architecture**. Пользовательские действия не просто вызывают локальные UI-реакции, а изменяют наблюдаемое состояние рабочей среды, из которого затем заново строятся и представление для User, и следующая операция AI. React в этом смысле полезен как понятный культурный ориентир: он приучил индустрию мыслить через состояние и его производные представления. Но для CAV этого уже недостаточно. Здесь state не должен замыкаться внутри интерфейса, потому что он является частью самой рабочей среды: внешней, накопительной и доступной не только Viewer, но и агенту как элементу общего Context.
